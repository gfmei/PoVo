...
# from llava.vlm_utils import load_llava_model, generate_category_from_llava
...
# def save_cls_text_from_imgs(data_dir, split, output_path):
#     if split == 'train':  # for training set, export a chunk of point cloud
#         scannet_file_list = read_files('../../llava/meta_data/scannetv2_train.txt')
#     else:  # for the validation set, export the entire point cloud instead of chunks
#         scannet_file_list = read_files('../../llava/meta_data/scannetv2_val.txt')
#
#     data_root_2d = join(data_dir, 'scannet_2d')
#     tokenizer, model, image_processor, conv = load_llava_model(model_path="liuhaotian/llava-v1.6-mistral-7b")
#     for scene_id in scannet_file_list:
#         img_dirs = sorted(glob(join(data_root_2d, split, scene_id, 'color/*')),
#                           key=lambda x: int(os.path.basename(x)[:-4]))
#         if scene_id in train_scenes:
#             output_file = os.path.join(output_path, 'train', f'{scene_id}_llava.txt')
#             split_name = 'train'
#         elif scene_id in val_scenes:
#             output_file = os.path.join(output_path, 'val', f'{scene_id}_llava.txt')
#             split_name = 'val'
#         else:
#             output_file = os.path.join(output_path, 'test', f'{scene_id}_llava.txt')
#             split_name = 'test'
#         print(f'Processing: {scene_id} in {split_name}')
#         k = min(len(img_dirs), 32)
#         img_dirs = random.sample(img_dirs, k)
#         labels = generate_category_from_llava(img_dirs, tokenizer, model, image_processor, conv)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_root', default='/storage3/TEV/eturri/datasets/ScanNet',
                        help='Path to the ScanNet dataset containing scene folders')
    parser.add_argument('--output_root', default='/storage2/TEV/datasets/ScanNet',
                        help='Output path where train/val folders will be located')
    parser.add_argument('--parse_normals', default=True, type=bool, help='Whether parse point normals')
    config = parser.parse_args()

    # Load label map
    labels_pd = pd.read_csv('./meta_data/scannetv2-labels.combined.tsv',
                            sep='\t', header=0)
    ...
    with ProcessPoolExecutor(max_workers=mp.cpu_count()) as executor:
        # Load scene paths
        scene_paths = sorted(glob(config.dataset_root + '/scans*/scene*'))
        ...
        _ = list(executor.map(handle_process, scene_paths, repeat(config.output_root), repeat(labels_pd),
                              repeat(train_scenes), repeat(val_scenes), repeat(config.parse_normals)))

    # save_cls_text_from_imgs(config.dataset_root, 'val', config.output_root)
